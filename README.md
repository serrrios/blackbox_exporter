...
This fork also includes an `openssl_http` prober for Linux deployments where the
HTTP request and TLS handshake must be performed through the OpenSSL CLI backend
instead of Go's native TLS stack.
...
HTTP, HTTPS (via the `http` prober), HTTPS via the `openssl_http` prober, DNS, TCP socket, ICMP and gRPC (see permissions section) are currently supported.
...
HTTP, TCP, DNS, and **openssl_http** probes can accept an additional `hostname` query parameter (Prometheus: `__param_hostname`):
- For HTTP probes: sets `Host` header and TLS SNI
- For **openssl_http**: sets `Host` header and `tls_config.server_name` (SNI) when not already set in the module — same as HTTP
- For TCP probes: sets TLS SNI if not already configured
- For DNS probes: uses as `query_name` for DNS queries

Do **not** set `__param_hostname` from the blackbox scrape address (`__address__` of the exporter, e.g. `ext-proxy-zbx:9116`): that value is the machine running blackbox, not the site you probe. Use the vhost from the target URL, `__meta_dns_name`, or a dedicated label derived from the probed service.

This can be especially useful with `dns_sd_config`:
```yaml
scrape_configs:
